...
## 1.1.0 - Unreleased
> {warning} The `s-vars` attribute has been deprecated for security reasons, use the new `sprig.vals()` function instead. The magic `_url` and `_events` variables have been removed, use the new `sprig.pushUrl()` and `sprig.triggerEvents()` functions instead.

### Added
- Added a [sprig.vals()](https://putyourlightson.com/plugins/sprig#sprig.valsvalues) template variable that should be used instead of the deprecated [s-vars](https://putyourlightson.com/plugins/sprig#s-vars).
...
### Deprecated
- The `s-vars` attribute has been deprecated for security reasons. Use the new `sprig.vals()` function instead.
